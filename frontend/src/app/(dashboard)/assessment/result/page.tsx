...
const DOMAIN_META: Record<string, { label: string; color: string; badge: string }> = {
  web:      { label: "Web Developer",       color: "#344256", badge: "badge-web"  },
  ai:       { label: "AI/ML Engineer",      color: "#3f5f86", badge: "badge-ai"   },
  data:     { label: "Data Engineer",       color: "#4f7f7a", badge: "badge-data" },
  devops:   { label: "DevOps Engineer",     color: "#9a7646", badge: "badge-intermediate" },
  undecided:{ label: "Chưa xác định",       color: "#8892A4", badge: "" },
};
...
export default function AssessmentResultPage() {
  ...
  const fetchAISuggestion = useCallback(async (r: AssessmentResult) => {
    setLoadingAI(true);
    try {
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(() => fetchAISuggestion(result));
  }, [fetchAISuggestion, result, router]);
              ...
              <div key={domain} style={{ marginBottom: 14 }}>
                ...
                <div style={{ position: "relative", height: 8, background: "rgba(255,255,255,0.07)", borderRadius: 4, overflow: "visible" }}>
                  ...
                  <div style={{
                    ...
                    background: dm?.color || "var(--accent-purple)",
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            ...
            {!loadingAI && suggestion?.gemini_analysis && (
              <div style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6, fontStyle: "italic", marginBottom: 12, padding: "10px 14px", background: "rgba(255,255,255,0.05)", borderRadius: 8, borderLeft: `3px solid ${domainMeta.color}` }}>
                &ldquo;{suggestion.gemini_analysis}&rdquo;
              </div>
            )}
